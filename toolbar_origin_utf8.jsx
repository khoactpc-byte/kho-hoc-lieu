...
export default function ThdTeachingAssignmentsToolbar({
  ...
  teachingFilter = 'all',
  teachingFilterLabel = 'Tß║Ñt cß║ú',
    ...
    <div className="relative z-[260] flex-none rounded-b-none rounded-t-none border-x border-b border-cyan-100 bg-white/95 px-2 py-1 shadow-sm backdrop-blur">
      <div className="flex flex-wrap items-center gap-1.5">
        <div className="min-w-28">
          <div className="text-[10px] font-semibold uppercase text-cyan-900">Ph├ón c├┤ng</div>
        ...
        {isThdTeachingPanel && (
          <div className="flex flex-wrap items-center gap-1.5 rounded-md border border-indigo-100 bg-indigo-50/60 px-1.5 py-1">
            ...
            {!activeTeachingBatch && (
              <>
                <TeachingImportDateFields
                  ...
                  title="Ng├áy bß║»t ─æß║ºu nhß║¡p file"
                  schoolYear={selectedSchoolYear}
                />
                <span className="text-xs font-semibold text-indigo-700">─æß║┐n</span>
                <TeachingImportDateFields
                  ...
                  title="Ng├áy kß║┐t th├║c nhß║¡p file"
                  schoolYear={selectedSchoolYear}
                />
              </>
            )}
            <button type="button" onClick={openTeachingImportFilePicker} disabled={!canUseTeachingImport} className="inline-flex h-7 items-center gap-1.5 rounded-md border border-indigo-200 bg-white px-2.5 text-xs font-semibold text-indigo-800 hover:bg-indigo-100 disabled:cursor-not-allowed disabled:opacity-40">
              <FileSpreadsheet className="h-4 w-4" /> Th├¬m dß╗» liß╗çu
            </button>
          </div>
        )}
        {hasTeachingBatches && (
          <label className="flex h-8 items-center gap-1.5 rounded-md border border-slate-200 bg-slate-50 px-2 text-xs font-semibold text-slate-700">
            ...
            >
              <option value="summary">Tß╗òng hß╗úp ({teachingBatchesForSelectedYear.length} ─æß╗út)</option>
        ...
        {isTeachingSummaryView && (
          ...
          >
            <ClipboardCheck className="h-4 w-4" /> Cß║¡p nhß║¡t{teachingSummaryDirty ? ' *' : ''}
          </button>
        )}
        {isTeachingSummaryView && (
          <button
          ...
          >
            <Pencil className="h-4 w-4" /> Chß╗ënh sß╗¡a
          </button>
        )}
        {activeTeachingBatch && (
          <>
            ...
            >
              <Pencil className="h-4 w-4" /> {isEditingActiveTeachingBatch ? '─Éang chß╗ënh sß╗¡a' : 'Chß╗ënh sß╗¡a'}
            </button>
            <button
              type="button"
            ...
            >
              GV mß╗¢i: {newTeachersComparedToPreviousBatch.length}
            </button>
            {isEditingActiveTeachingBatch && (
            <button
              type="button"
            ...
            >
              <Trash2 className="h-4 w-4" /> X├│a ─æß╗út
            </button>
            )}
          </>
        )}
        <div className="ml-auto flex flex-wrap gap-1.5">
          ...
          >
            <CalendarDays className="h-4 w-4" /> C├ái ─æß║╖t
          </button>
          <button
            type="button"
            ...
            className="inline-flex h-8 items-center gap-1.5 rounded-md border border-amber-200 bg-amber-50 px-2.5 text-xs font-semibold text-amber-800 hover:bg-amber-100"
          >
            <ClipboardCheck className="h-4 w-4" /> Kiß╗âm tra
          </button>
          {!isThdTeachingPanel && (
            <button
              type="button"
            ...
            >
              <Pencil className="h-4 w-4" /> {isEditingMainTeaching ? '─Éang chß╗ënh sß╗¡a' : 'Chß╗ënh sß╗¡a'}
            </button>
          )}
          {!isThdTeachingPanel && (
            <button
            ...
            >
              {showTeachingMoneyColumns ? 'ß║¿n tiß╗ün' : 'Hiß╗çn tiß╗ün'}
            </button>
          )}
          <div className="relative">
            ...
            >
              <Download className="h-4 w-4" /> Xuß║Ñt file
            </button>
          ...
          {isThdTeachingPanel && (
            <>
              <div className="relative">
                ...
                >
                  <Filter className="h-4 w-4" /> Lß╗ìc: {teachingFilterLabel}
                </button>
