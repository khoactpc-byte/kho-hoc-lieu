...
export default function AdminSettingsWorkspace({
  ...
  const [teachingCheckResultFilter, setTeachingCheckResultFilter] = useState([]);
  ...
  const handleCreateNewTeachingBatch = useCallback(() => {
    ...
    setThdTeachingAssignmentsDirty(true);
  }, [activeTeachingDataKey, setActiveTeachingAssignmentsDraft]);

  const teachingBatchesForSelectedYear = useMemo(() => {
    const rows = activeTeachingAssignmentsDraft?.batchesByYear?.[activeTeachingDataKey];
  ...
  const teachingSemesterDatesForYear = useMemo(() => (
    normalizeTeachingSemesterDates(activeTeachingAssignmentsDraft?.semestersByYear?.[activeTeachingDataKey], selectedSchoolYear)
  ...
  const deleteSelectedTeachingBatch = () => {
    ...
    setActiveTeachingAssignmentsDraft(prev => {
      ...
      return {
        ...
        byYear: {
          ...(prevObj.byYear || {}),
          [effectiveSchoolYearKey]: nextBatches.length ? (prevObj.byYear?.[activeTeachingDataKey] || []) : []
        }
      };
    });
    setSelectedTeachingBatchId('summary');
  ...
  const formatSubjectLabel = (subject) => {
    ...
    const normalized = normalizeTeacherNameKey(s);
    if (normalized.includes('c nghe')) return 'Công nghệ';
    if (normalized.includes('an')) return 'Âm nhạc';
    if (normalized.includes('mt')) return 'Mĩ thuật';
    if (normalized === 'van') return 'Ngữ văn';
    if (normalized.includes('van ts')) return 'Ngữ văn (TS 10)';
    if (normalized.includes('toan ts')) return 'Toán (TS 10)';
    if (normalized.includes('anh ts')) return 'Tiếng Anh (TS 10)';
    return s;
  };
  ...
  const uniqueAssignmentSubjects = useMemo(() => {
    if (!teachingRowsForSelectedYear) return [];
    const subjects = new Set();
    teachingRowsForSelectedYear.forEach(row => {
      if (row.assignments) {
        row.assignments.forEach(assignment => {
          if (assignment.subject) subjects.add(assignment.subject);
        });
      }
    });
  ...
  const filteredTeachingResult = useMemo(() => {
  ...
  const teachingFilterLabel = !teachingFilter || teachingFilter.length === 0 || teachingFilter.includes('all') 
    ? 'Tất cả' 
        ...
        <div className={isTeachingPanel ? 'min-h-0 flex-1 overflow-hidden' : 'space-y-3'}>
            ...
            {isTeachingPanel && (
              <ThdTeachingAssignmentsPanel
                ...
                teachingFilterOptions={[...TEACHING_FILTER_OPTIONS, ...dynamicSubjectFilterOptions]}
